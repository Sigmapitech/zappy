...
bool assign_team(server_t *srv, const event_t *event)
{
    ...
    for (size_t i = 0; i < srv->cstates.nmemb; i++)
        if (srv->cstates.buff[i].fd == event->id
    ...
    if (team_id == GRAPHIC_TEAM_ID)
        send_starting_data_graphic(srv, event->id);
    else
        send_starting_data_ai(srv, event->id);
    return true;
}
...
{
    if (event->id == srv->self_fd)
        return;
    if (!assign_team(srv, event))
        append_to_output(srv, &srv->cstates.buff[event->id], "ko\n");
}
